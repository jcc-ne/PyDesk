...
class XLtable(pd.DataFrame):
    ...
    def change_colStyle(self, ws, xcol=0, style=Style.default_style):
        """
        Write XLtable data into an Excel sheet
        ...
        """
        for irow in range(len(self.index)):
            ws.write(irow, xcol,
                     self.ix[self.index[irow]][self.columns[xcol]], style)
#        for irow in range(len(self.index)): # data
#            ws.row(irow).write(xcol,
#                               self.ix[self.index[irow]][self.columns[xcol]],
#                               style)
